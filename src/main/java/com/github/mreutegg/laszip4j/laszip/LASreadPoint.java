...
public class LASreadPoint {
   ...
   public boolean setup(int num_items /*unsigned*/, LASitem[] items, LASzip laszip) {
    if (laszip != null && (num_items != laszip.num_items || items != laszip.items)) {
        return false;
    }
    ...
    for (int i = 0; i < num_readers; i++) {
        ...
        switch (items[i].type) {
            case POINT10:
                reader = new LASreadItemRaw_POINT10();
                break;
            case GPSTIME11:
                reader = new LASreadItemRaw_GPSTIME11();
                break;
            case RGB12:
            case RGB14:
                reader = new LASreadItemRaw_RGB12();
                break;
            case WAVEPACKET13:
            case WAVEPACKET14:
                reader = new LASreadItemRaw_WAVEPACKET13();
                break;
            case BYTE:
            case BYTE14:
                reader = new LASreadItemRaw_BYTE(items[i].size);
                break;
            case POINT14:
                reader = new LASreadItemRaw_POINT14();
                break;
            case RGBNIR14:
                reader = new LASreadItemRaw_RGBNIR14();
                break;
            default:
                return false;
        }
        readers_raw[i] = reader;
    ...
    if (dec != null) {
        ...
        for (int i = 0; i < num_readers; i++) {
            PointDataRecord seekPoint;
            LASreadItemCompressed reader;
            switch (items[i].type) {
                case POINT10:
                    seekPoint = new PointDataRecordPoint10();
                    if (items[i].version == 1) {
                        reader = new LASreadItemCompressed_POINT10_v1(dec);
                    } else if (items[i].version == 2) {
                        reader = new LASreadItemCompressed_POINT10_v2(dec);
                    } else {
                        return false;
                    }
                    break;
                case GPSTIME11:
                    seekPoint = new PointDataRecordGpsTime();
                    if (items[i].version == 1) {
                        reader = new LASreadItemCompressed_GPSTIME11_v1(dec);
                    } else if (items[i].version == 2) {
                        reader = new LASreadItemCompressed_GPSTIME11_v2(dec);
                    } else {
                        return false;
                    }
                    break;
                case RGB12:
                    seekPoint = new PointDataRecordRGB();
                    if (items[i].version == 1) {
                        reader = new LASreadItemCompressed_RGB12_v1(dec);
                    } else if (items[i].version == 2) {
                        reader = new LASreadItemCompressed_RGB12_v2(dec);
                    } else {
                       return false;
                    }
                    break;
                case RGB14:
                    ...
                    if (items[i].version == 4 || items[i].version == 3 || items[i].version == 2) {
                        reader = new LASreadItemCompressed_RGB14_v3(dec, decompress_selective);
                    } else {
                        return false;
                    }
                    break;
                case WAVEPACKET13:
                    seekPoint = new PointDataRecordWavepacket();
                    if (items[i].version == 1) {
                        reader = new LASreadItemCompressed_WAVEPACKET13_v1(dec);
                    } else {
                        return false;
                    }
                    break;
                case WAVEPACKET14:
                    ...
                    if (items[i].version == 4 || items[i].version == 3) {
                        reader = new LASreadItemCompressed_WAVEPACKET14_v3(dec, decompress_selective);
                    } else {
                        return false;
                    }
                    break;
                case BYTE:
                    seekPoint = new PointDataRecordBytes(items[i].size);
                    if (items[i].version == 1) {
                        reader = new LASreadItemCompressed_BYTE_v1(dec, items[i].size);
                    } else if (items[i].version == 2) {
                        reader = new LASreadItemCompressed_BYTE_v2(dec, items[i].size);
                    } else {
                        return false;
                    }
                    break;
                case BYTE14:
                    seekPoint = new PointDataRecordBytes(items[i].size);
                    if (items[i].version == 4 || items[i].version == 3) {
                        reader = new LASreadItemCompressed_BYTE14_v3(dec, items[i].size, decompress_selective);
                    } else {
                        return false;
                    }
                    break;
                case POINT14:
                    seekPoint = new PointDataRecordPoint14();
                    if (items[i].version == 4 || items[i].version == 3 || items[i].version == 2) {
                        reader = new LASreadItemCompressed_POINT14_v3(dec, decompress_selective);
                    } else {
                        return false;
                    }
                    break;
                case RGBNIR14:
                    seekPoint = new PointDataRecordRgbNIR();
                    if (items[i].version == 4 || items[i].version == 3 || items[i].version == 2) {
                        reader = new LASreadItemCompressed_RGBNIR14_v3(dec, decompress_selective);
                    } else {
                        return false;
                    }
                    break;
            ...
            seek_point[i] = seekPoint;
            readers_compressed[i] = reader;
        }
        if (laszip.compressor != LASZIP_COMPRESSOR_POINTWISE && laszip.chunk_size != 0) {
