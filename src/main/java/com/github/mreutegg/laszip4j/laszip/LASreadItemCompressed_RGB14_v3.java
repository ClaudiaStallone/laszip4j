...
public class LASreadItemCompressed_RGB14_v3 extends LASreadItemCompressed{
 ...
 public PointDataRecord read(int context) {
    ...
    if (changed_RGB) {
        ...
        result.R = (char) ((sym & 1) != 0 ? U8_FOLD(dec_RGB.decodeSymbol(contexts[current_context].m_rgb_diff_0)) + (last_item.R & 255) : last_item.R & 0xFF);
        result.R |= (char) ((sym & 2) != 0 ? U8_FOLD(dec_RGB.decodeSymbol(contexts[current_context].m_rgb_diff_1)) + (last_item.R >>> 8) : last_item.R & 0xFF00);

        if ((sym & 64) != 0) {
            int diff = (result.R & 0xFF) - (last_item.R & 0xFF);

            result.G = (char) (((sym & 4) != 0 ? U8_FOLD(dec_RGB.decodeSymbol(contexts[current_context].m_rgb_diff_2)) + U8_CLAMP(diff + (last_item.G & 255)) : last_item.G & 0xFF));
            result.B = (char) (((sym & 16) != 0 ? U8_FOLD(dec_RGB.decodeSymbol(contexts[current_context].m_rgb_diff_4)) + U8_CLAMP((diff + (result.G & 0xFF) - (last_item.G & 0xFF))) / 2 + (last_item.B & 255) : last_item.B & 0xFF));

            diff = (result.R >>> 8) - (last_item.R >>> 8);

            result.G |= (char) (((sym & 8) != 0 ? U8_FOLD(dec_RGB.decodeSymbol(contexts[current_context].m_rgb_diff_3)) + U8_CLAMP(diff + (last_item.G >>> 8)) : last_item.G & 0xFF00));
            result.B |= (char) (((sym & 32) != 0 ? U8_FOLD(dec_RGB.decodeSymbol(contexts[current_context].m_rgb_diff_5)) + U8_CLAMP((diff + (result.G >>> 8) - (last_item.G >>> 8))) / 2 + (last_item.B >>> 8) : last_item.B & 0xFF00));
        } else {
            result.G = result.R;
            result.B = result.R;
        }
